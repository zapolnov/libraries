...
ILint XpmGetsInternal(SIO* io, ILubyte *Buffer, ILint MaxLen)
{
	...
	while ((Current = io->getc(io)) != IL_EOF && i < MaxLen - 1) {
		if (Current == IL_EOF)
			return 0;
		if (Current == '\n') //unix line ending
			break;
...
ILboolean iIsValidXpm(SIO* io)
{
	...
	io->seek(io, Pos, IL_SEEK_SET);  // Restore position
...
{
	ILuint Index = XpmHash(Name, Len);
	//@todo: check whether Index is legal?
	XPMHASHENTRY* Entry = Table[Index];
...
ILboolean XpmPredefCol(char *Buff, XpmPixel *Colour)
{
	...
	if (!strcmp(Buff, "black")) {
		(*Colour)[0] = 0;
		(*Colour)[1] = 0;
		(*Colour)[2] = 0;
		return IL_TRUE;
	}
	if (!strcmp(Buff, "white")) {
		(*Colour)[0] = 255;
		(*Colour)[1] = 255;
		(*Colour)[2] = 255;
		return IL_TRUE;
